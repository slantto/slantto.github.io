...
S_Hokuyo=Init_Hokuyo(4);    % initalize the Lidar
pause(0.1);
fscanf(S_Hokuyo)
...
pause(0.1);
   ...
   turncount=0;
   turnflag=0; %0 for left 1 for right
   SD.thetar=zeros(1,600);
   A=1;
   X_P=0;
   P_P=1;
...
%     drawnow
    ...
    SD.thetaL(i)=acosd(900/(SD.Laser_RF(i)-155));
    if i==1
        SD.thetar(i)=0+(SD.R(i)+0.0028)*SD.Time_Diff(i);
    else
        SD.thetar(i)=SD.thetar(i-1)+(SD.R(i)+0.0028)*SD.Time_Diff(i);
    end
    
    Q=(SD.Time_Diff(i)^2)*2.6312e-5;
    R=10.3413;
    %Prediction
    X_A=X_P-SD.Time_Diff(i)*(SD.R(i)-0.0028)+Q;
    P_A=A*P_P*A'+Q;
    
    %Measurement update
    H=900*sec(X_A)*tan(X_A);
    y=SD.Laser_RF(i)+R;
    r=y-(900*sec(X_A)+155);
    S=H*P_A*H'+R;
    K=P_A*H'*inv(S);
    ...
    if turncount<6
        ...
        if turnflag==0
            SetDriveWheelsSMART(S_Create, Create_Full_Speed*.2, Create_Full_Speed*-.2, CliffLeft,CliffRight,CliffFrontLeft,CliffFrontRight,BumpRight,BumpLeft,BumpFront);
            if abs(SD.thetar(i)) >= deg2rad(30)
                turnflag=1;
            end
        else
            SetDriveWheelsSMART(S_Create, Create_Full_Speed*-.2, Create_Full_Speed*.2, CliffLeft,CliffRight,CliffFrontLeft,CliffFrontRight,BumpRight,BumpLeft,BumpFront);
        ...
        SetDriveWheelsSMART(S_Create, Create_Full_Speed*0, Create_Full_Speed*0, CliffLeft,CliffRight,CliffFrontLeft,CliffFrontRight,BumpRight,BumpLeft,BumpFront);
    end
    %----------------------------------------------------------
