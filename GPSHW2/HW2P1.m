...
for i=1:N
    ...
    %elevtation dependent mapping function 
    mel=1/(sqrt(1-((cosd(el(:,i))/1.001).^2)));
...
for i=1:N
    ...
    %Use function LLSPos to get xyz estimate at each epoch
    [xyzEstP2(:,i),clockBiasEstP2(:,i),PDOP(:,i),TDOP(:,i),GDOP(:,i)]=LLSPos(Satxyz,pr,nomXYZ,clockBiasNom,Nsats,c,W,trop,ion);
    %substitute xyz estimate for epoch i to be xyz nom for epoch i+1
    nomXYZ=xyzEstP2(:,i)';
