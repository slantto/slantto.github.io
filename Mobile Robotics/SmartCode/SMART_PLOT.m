...
% 
% figure;
% plot(Time_Vector,rad2deg(SD.TotalAngle),Time_Vector,rad2deg(-SD.Yaw),Time_Vector,rad2deg(-SD.thetar),Time_Vector,SD.thetaL,Time_Vector,rad2deg(SD.ThetaEKF))
% axis tight; grid on;
% legend('Encoder','Yaw(w/o bias removal)','Yaw(w/ bias removal','laser','EKF');
% title('comparison of yaw angle estimates'); ylabel('Yaw(deg)'); xlabel('Time(s)');
% ylim([-25,60]);

figure;
plot(Time_Vector,SD.thetaL)
axis tight; grid on;
title('LIDAR Yaw angle estimates'); ylabel('Yaw(deg)'); xlabel('Time(s)');
