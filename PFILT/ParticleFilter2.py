...
New to this version is the prediction model takes an input form nov span for NED velocities
"""
...
if __name__ == '__main__':
    ...
    particles, weights, N = neo.create_uniform_particles(nrange, erange, rad_range, 25000)  # Uniform Distribution
    ...
    for ii in np.arange(0, truthNED.shape[0]):
        ...
        if picsave == 10:
            ...
            parts = ax.scatter(particles[:, 1], particles[:, 0], s=10, c=weights, cmap='plasma',
                               label='Particles')
            ax.scatter(truthNED[ii, 1], truthNED[ii, 0], s=25, marker='^', facecolor='g', edgecolors='g',
                       label='True Pos.')
            ax.scatter(posmean[ii][1], posmean[ii][0], s=25, marker='x', facecolor='r', edgecolors='r',
                       label='Est. Pos.')
            ...
            colbar.set_label('Particle Weights')
            plt.xlabel('E(meters)')
            plt.ylabel('N(meters)')
