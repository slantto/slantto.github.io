...
import neogeodb.hdf_orthophoto as hdo
import cv2
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--vrt', action='store_true')

    args = parser.parse_args()

    # Set up terrain handlers, feature detectors/extractors
    srtm = SRTM(args.terrain, args.geoid)
    # detector, desc_extract = f2d.load_feature_operators(
    ...
    if args.vrt:
        ophoto = hdo.VRTOrthophoto(args.ophoto)
    else:
        ophoto = hdo.HDFOrthophoto(args.ophoto, img_path=args.img)


    # Compute windows into the HDF Orthophoto
    slc_cnt = 0
    t0 = time.time()
    slices = tbdb.slices_from_ophoto(ophoto, args.dim)
    for slc in slices:
        t1 = time.time()
        print('Working on slice %s: %d / %d' % (slc, slc_cnt, len(slices)))
        df, dg = tbdb.features_from_slice(ophoto, slc,
