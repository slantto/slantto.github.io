...
class PartycleFilt(object):
    ...
    def motion_model(self, n_e_delta_pos, delta_pos_sigma, rad_sigma):
        ...
        move_d = dx[1] + (randn(n) * delta_pos_sigma[2])
        self.particles[:, 0] += move_n
        self.particles[:, 1] += move_e
        self.particles[:, 2] += move_d
    ...
    def estimate_pos(self):
        ...
        covariance = np.dot((self.weights[:]*((self.particles[:] - particle_mean).transpose())), (self.particles[:] - particle_mean))
